...
2. AI reads `docs/00-ai-rules.md` (and lazy-loads relevant docs if needed).
3. AI scopes the work and creates/enters a `work/X.Y/` directory.
...
- **`docs/`**: Acts as the long-term memory for the AI. It MUST stay clean, concise, and heavily curated to avoid token limits.
- **`work/`**: Acts as the short-term working memory. It stores the messy, step-by-step history so the agent doesn't need to keep it in its context window. It also serves as an audit trail for the human.
